    ...
    for task_type in ["fast", "general", "coding", "planning"]:
        ...
        try:
            ...
            if r.status_code == 200 and d.get("success"):
                ok(f"Response from {d.get('provider')}/{d.get('model_name')}")
                ...
                fail(f"taskType={task_type} failed", d.get("error", str(d)))
                results["failed"] += 1
        except Exception as e:
...
def test_workflow():
    ...
    try:
        ...
        check(d.get("status") == "planned", "Status = planned",                           f"Status = {d.get('status')}")
