...
interface UseWorkflowExecutionResult {
  ...
  isLoading: boolean;
  error: string | null;
  startExecution: (prompt: string) => Promise<WorkflowExecution | null>;
...
export function useWorkflowExecution(): UseWorkflowExecutionResult {
  ...
  const startExecution = useCallback(
    async (prompt: string): Promise<WorkflowExecution | null> => {
      try {
        setIsLoading(true);
        setError(null);

        console.log('[v0] Starting workflow execution for:', prompt);
